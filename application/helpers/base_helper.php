...
if ( ! function_exists('get_site_logo'))
{
    ...
    {
        $logo = get_settings('site_settings','site_logo','http://placehold.it/200x200');
...
if ( ! function_exists('is_store_owner'))
{
    function is_store_owner()
    {
        $CI = get_instance();
        if($CI->session->userdata('user_email')!='' && $CI->session->userdata('user_type')==3)
            return TRUE;
        else
            return FALSE;
    }
...
if ( ! function_exists('is_employee'))
{
    ...
    {
        $CI = get_instance();
        if($CI->session->userdata('user_email')!='' && $CI->session->userdata('user_type')==4)
            return TRUE;
        else
            return FALSE;
    }
...
if ( ! function_exists('is_customer'))
{
    ...
    {
        $CI = get_instance();
        if($CI->session->userdata('user_email') != '' && $CI->session->userdata('user_type') == 5)
            return TRUE;
        else
            return FALSE;
    }
...
if ( ! function_exists('lang_key'))
{
    function lang_key($key = '')
    {
        if(defined('LANG_ARRAY'))
        {
            ...
            /*$CI =&  get_instance();
            $CI->load->database();
             $check_phrase	=	$CI->db->get_where('language' , array('phrase' => $key))->row()->phrase;
             if ( $check_phrase	!=		$key)
             	$CI->db->insert('language' , array('phrase' => $key));*/
        ...
        else
        {
            ...
            {
                if(!@file_exists(FCPATH."/locals/".$current_language.'.yml'))
                {
                    $file_name = $default_lang.'.yml';
                }
                else
                {
                    $file_name = $current_language.'.yml';
                }
            }
...
if ( ! function_exists('get_db_field_by_id'))
{
    function get_db_field_by_id($db = '', $db_field = '', $id = '') {
        $CI = get_instance();
        $CI->load->database();
        return $CI->db->get_where($db, array($db . '_id' => $id))->row()->$db_field;
    }
}
