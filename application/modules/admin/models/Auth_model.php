...
class Auth_model extends CI_Model
{
	...
	function check_login($email,$password,$return_as='num_rows')
	{
	...
	function set_login_cookie($user_name)
	{
		...
		$this->db->update('user',$data,array('user_name'=>$user_name));
	}
	
	function check_cookie_val($user,$key)
	{
		$query = $this->db->get_where('user',array('user_name'=>$user,'remember_me_key'=>$key));
		if($query->num_rows()>0)
		{
	...
	function set_recovery_key($user_email)
	{
		...
		$this->db->update('user',$data,array('user_email'=>$user_email));
	...
	function verify_recovery($user_name,$recovery_key)
	{
		...
		{
			$query = $this->db->get_where('user',array('user_name'=>$user_name,'recovery_key'=>$recovery_key));
			return $query->num_rows();
		}
	}
	...
	{
		$query = $this->db->get_where('user',array('user_email'=>$email,'confirmation_key'=>$code));
		if($query->num_rows()>0)
		{
			...
			$this->db->update('user',$data,array('user_email'=>$email));
			return TRUE;
		}
		else
		{
