...
class Auth extends CI_Controller {
    ...
    function login() {
        ...
        $query                              = $this->auth_model->check_login($email, $password, 'result');
        ...
        if ($query->num_rows() > 0) {
            ...
            if ($ok == 1) {
                ...
                if ($this->session->userdata('req_url') != '') {
                    $response['redirect_url'] = $this->session->userdata('req_url');
                } else {
                    $response['redirect_url'] = site_url('admin');
                }
            } else
                $response['invalid_login_message'] = 'Account Banned';
        } else {
...
//		delete_cookie('key','localhost','/','mycookie_');
    ...
    function ajax_forgot_password() {
        ...
        $this->email_model->notify_email('password_reset_confirmation', $email, $new_password);
        
        $this->session->set_flashdata('flash_message', lang_key('password_has_been_reset') . '. ' . lang_key('please_check_your_email') . '.');
        redirect(site_url('admin/auth'));
    ...
    {
        //echo $this->db->get_where('country', array('id' => $country_id))->row()->calling_code;
    ...
    {
        if($this->db->get_where('user', array('phone' => $phone))->num_rows() > 0)
            echo 'invalid';
        else
            echo 'valid';
    }
    
    function check_email_validity_2($email = '', $user_id = '')
    {
        if($this->db->get_where('user', array('user_id !=' => $user_id, 'email' => $email))->num_rows() > 0)
            echo 'invalid';
        else
            echo 'valid';
    }
    
    function check_phone_validity_2($phone = '', $user_id = '')
    {
        if($this->db->get_where('user', array('user_id !=' => $user_id, 'phone' => $phone))->num_rows() > 0)
            echo 'invalid';
        else
            echo 'valid';
    }
