...
<?php echo form_open(site_url('reporting/expense_report') , array('class' => 'form-horizontal form-groups validate',  'id' => 'date_selector_form'));?>
...
if(!empty($expenses))
    foreach($expenses as $row)
        if($timestamp_start <= $row['date'] && $timestamp_end >= $row['date'])
            array_push($total_expenses, array('date' => $row['date'], 'title' => $row['title'],
                'account_id' => $row['account_id'], 'amount' => $row['amount']));
        
if(!empty($purchases))
    foreach($purchases as $row)
        if($timestamp_start <= $row['due_date'] && $timestamp_end >= $row['due_date'])
            array_push($total_expenses, array('date' => $row['due_date'], 'title' => lang_key('purchase_code') . ' : ' . $row['purchase_code'],
                'account_id' => $row['account_id'], 'amount' => $row['amount']));

if(!empty($total_expenses))
    foreach($total_expenses as $row)
        $amount += $row['amount'];
?>
...
<?php } else { ?>
    ...
    <table class="table table-bordered responsive datatable" id="table_export">
        ...
        <tbody>
            ...
            foreach($total_expenses as $key => $row)
                $date[$key] = $row['date'];
            
            array_multisort($date, SORT_DESC, $total_expenses);
...
<script>
    ...
    var chart = AmCharts.makeChart("bar_chartdiv", {
        ...
        "dataProvider"  : [
            ...
            if(!empty($expenses))
                foreach($expenses as $row)
                    array_push($total_expenses, array('date' => $row['date'], 'amount' => $row['amount']));

            if(!empty($purchases))
                foreach($purchases as $row)
                    array_push($total_expenses, array('date' => $row['due_date'], 'amount' => $row['amount']));
            
            foreach($total_expenses as $row) {
                $expense_year = date('Y', $row['date']);
                
                if($financial_year_start == 'january') {
                    if($expense_year == $current_year) {
                        $expense_month = date('n', $row['date']) - 1;
                        $monthwise_amount[$expense_month]['amount'] += $row['amount'];
                    }
                } else {
        ...
        <div class="panel panel-primary">
            <div class="panel-heading">
                <div class="panel-title" style="width: 100%;">
                    ...
                    <?php echo lang_key('expense_bar') . $time_range; ?>
        ...
        <div class="panel panel-primary">
            <div class="panel-heading">
                <div class="panel-title" style="width: 100%;">
                    ...
                    <?php echo get_phrase('expense_percentage') . $time_range; ?>
