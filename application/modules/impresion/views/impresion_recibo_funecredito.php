<?php
$sql = "SELECT s.service_id, CONCAT(s.client_first_name, ' ', s.client_last_name1, ' ', s.client_last_name2) AS name, s.client_id_card AS id_card, s.contract_id FROM bk_service AS s WHERE s.type = 'funecredito'";
        ...
        <tbody>
            ...
            foreach ($services as $row): ?>
                <tr>
                    ...
                    <td>
                        <button type="button" class="btn btn-default" onclick="showAjaxModal('<?php echo site_url('admin/modal/popup/impresion/recibo_dinero_funecredito/' . $row['service_id'] ); ?>');">
                            Imprimir recibo
                        </button>
                        <a type="button" class="btn btn-default" href="<?php echo site_url('/impresion/impresiones/historial?id='.$count); ?>">
                            Ver Historial de pagos
                        </a>
                        <a type="button" class="btn btn-default" href="<?php echo site_url('/impresion/impresiones/aplicar?tipo=funecredito?id='.$count); ?>">
                            Aplicar Pagos Pendientes
                        </a>
                        <a type="button" class="btn btn-danger" ); href="<?php echo site_url('/impresion/impresiones/revertir?tipo=funecredito?id='.$count); ?>">
                            Revertir Pagos
                        </a>
                    </td>
                </tr>
