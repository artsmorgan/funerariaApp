<?php 
$sql = "SELECT s.*, u.user_id AS seller_id, CONCAT( u.first_name, ' ', u.last_name ) AS seller_name, f.prima AS primaFunecredito, f.monto_total, f.monto_cuota, f.tiempo_servicio, f.interes
FROM bk_service AS s 
INNER JOIN bk_user AS u ON s.user_id = u.user_id 
INNER JOIN bk_funecredito_account AS f ON f.service_id = s.service_id
WHERE s.service_id = ?";
        ...
        <div class="panel panel-primary" data-collapsed="0">
            <div class="panel-heading">
                <div class="panel-title" >
                    ...
                    Actualizar servicio funecrédito 
                </div>
            </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <div class="col-sm-12">
                                <input type="text"  class="form-control format-currency" value="<?php echo $row['balance']; ?>" disabled>
                                <input type="hidden" name="balance" value="<?php echo $row['balance']; ?>">
                            </div>
                        </div>
                    </div>
                    <!-- col -->
                    <input type="hidden"  name="contrato_account_id1" id="contrato_account_id1" value="<?php echo $row['contrato_account_id1']; ?>"/>
                    <input type="hidden"  name="contrato_account_id2" id="contrato_account_id2" value="<?php echo $row['contrato_account_id2']; ?>"/>
                    <input type="hidden"  name="contrato_account_id3" id="contrato_account_id3" value="<?php echo $row['contrato_account_id3']; ?>"/>
                    <input type="hidden" name="saldoFunecredito" id="saldoFunecredito" value="<?php echo $row['monto_total']; ?>">
                    <input type="hidden" name="plazoFunecredito" id="plazoFunecredito" value="<?php echo $row['tiempo_servicio']; ?>">
                    <input type="hidden" name="interesFunecredito" id="interesFunecredito" value="<?php echo $row['interes']; ?>">
                    <input type="hidden" name="cuotaFunecredito" id="cuotaFunecredito" value="<?php echo $row['monto_cuota']; ?>">
                    <input type="hidden" name="primaFunecredito" id="primaFunecredito" value="<?php echo $row['primaFunecredito']; ?>">
                </div>
                <!-- fith row -->
...
<script>
   ...
   $('#pay1month').closest('.col-md-12').hide();
    ...
    $('#amountService').val( $('#amount').val() );
    $('#advance_payment').val( $('#primaFunecredito').val() );
    $('#debt').val( $('#saldoFunecredito').val() );
    $( '#plazo' ).val( $('#plazoFunecredito').val() );
    $( '#interes' ).val( $('#interesFunecredito').val() );
    $( '#cuota' ).val( $('#cuotaFunecredito').val() );
